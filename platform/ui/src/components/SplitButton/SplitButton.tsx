...
const classes = {
  Button: ({ isExpanded, primary }) =>
    classNames(
      baseClasses.Button,
      !isExpanded && !primary.isActive && 'hover:!bg-transparent hover:border-primary-dark'
    ),
    ...
    classNames(
      ...
      primary.isActive
        ? isExpanded
          ...
          : `${primary.isToggle
            ? 'border-secondary-dark bg-bg-black'
            : 'bg-black bg-black'
          }
            border-2 rounded-md !p-2` // Full, rounded border with less right padding when active.
        : `focus:!text-white focus:!rounded-md focus:!bg-black focus:!bg-black
        ${isExpanded
          ? 'border-primary-dark bg-transparent !text-primary-light'
          : 'border-secondary-dark bg-gray-900 group-hover/button:border-primary-dark group-hover/button:text-primary-light hover:!bg-transparent hover:border-primary-dark focus:!text-white'
        }
        `
    ),
  Secondary: ({ isExpanded, primary }) =>
    classNames(
      ...
      isExpanded
        ? 'bg-black !rounded-tr-md !rounded-br-md'
        : primary.isActive
          ? 'bg-gray-900'
          : 'hover:bg-transparent bg-gray-900 group-hover/button:border-primary-dark'
    ),
